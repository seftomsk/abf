...
func main() {
	...
	loginLimiter := limiter.NewLimiter(10, time.Minute)
	passwordLimiter := limiter.NewLimiter(100, time.Minute)
	ipLimiter := limiter.NewLimiter(1000, time.Minute)
	l := limiter.NewMultiLimiter(
		loginLimiter,
		passwordLimiter,
		ipLimiter)

	a := access.NewIPAccess(memory.Create())

	http.HandleFunc("/auth", getAuthHandler(a, l))
